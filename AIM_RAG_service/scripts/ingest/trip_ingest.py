"""
Manual trip ingest — edit CONFIG, then run:

  python -m scripts.ingest.trip_ingest

Uses Bedrock Titan embeddings only (no LLM).
Skips records when tripnumber already exists in the target collection.
...
DETAIL_KEYS = ("detailstrips", "details", "trips")
# ===============================================================
...
def load_trip_records(path: str) -> List[Dict[str, Any]]:
    ...
    if not isinstance(details, list):
        raise ValueError("Trip details must resolve to a list of records")
...
def is_recent_record(trip: Dict[str, Any], cutoff: datetime.datetime) -> bool:
    """True when the record's DATE_FIELD is on/after `cutoff`."""
...
def ingest_trips() -> Dict[str, Any]:
    records = load_trip_records(FILE_PATH)
    if not records:
        return {"ok": False, "error": "No trip records found", "path": FILE_PATH}
    ...
    for start in range(0, len(records), EMBED_BATCH_SIZE):
        ...
        for trip in chunk:
            ...
            key_value = trip.get(DUPLICATE_FIELD)
            key_str = (
                str(key_value).strip()
        ...
        if not pending:
            print(
                f"[trip ingest] processed "
                f"{min(start + len(chunk), len(records))} / {len(records)}"
            )
            continue

        texts = [
            _truncate_for_embedding(build_page_content(trip))
            for trip in pending
        ]
        ...
        for trip, vector in zip(pending, vectors):
            batch_docs.append(
                build_document(trip, vector, ingested_at, source_document)
            )
    ...
    return {
        ...
        "sample_tripid": (sample or {}).get(ID_FIELD),
        "sample_tripnumber": (sample or {}).get(DUPLICATE_FIELD),
